...
POLYMARKET_ROUND21_AI_SELECTION_DESIGN_SHA256 = (
    "4249bb64de61e01dd7dced9c8c0749d46e52ded8c634bcc903c3191db13a1231"
)
...
def validate_round21_ai_candidate_selection_design(
    ...
    if (
        ...
        != {
            ...
            "tertiary": (
                "minimize_maximum_drawdown_delta_fraction_across_81_ledgers"
            ),
            "quaternary": (
                "maximize_minimum_tail_mean_delta_quote_across_81_ledgers"
            ),
        ...
        != {
            "output": (
                "one_development_nominated_sealed_test_challenger_or_none"
            ),
...
class Round21AICandidateScore:
    ...
    def validated(self) -> Round21AICandidateScore:
        ...
        if (
            ...
            or _digest(self.model_digest, name="model digest")
            != self.model_digest
...
def select_round21_ai_candidate(
    ...
    if any(
        {(delta.profile, delta.scenario) for delta in value.deltas}
        != expected_ledgers
        for value in selected
    ):
        raise ValueError("Round 21 AI candidate ledger population differs")
    scores = tuple(
        _candidate_score(by_model[model])
    ...
    provisional = Round21AICandidateSelection(
        ...
        nominated_model_digest=(
            None if nominated is None else nominated.model_digest
        ),
