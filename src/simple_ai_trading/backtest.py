...
def _liquidity_session_adjustment(
    rows: List[ModelRow],
    index: int,
    cfg: StrategyConfig,
    base_threshold: float,
) -> tuple[float, float, bool, bool]:
    """Return threshold and sizing adjustments from observed liquidity only."""

    if not bool(getattr(cfg, "liquidity_risk_enabled", True)):
        return base_threshold, 1.0, False, False
    ...
    return _clamp_threshold(threshold), max(0.0, min(1.0, size_multiplier)), low_liquidity, low_dynamic_session
...
def run_backtest(
    ...
    *,
    ...
    for row_index, (row, raw_score) in enumerate(zip(rows, probabilities, strict=True)):
        ...
        adjusted_threshold, liquidity_size_multiplier, low_liquidity, low_dynamic_session = _liquidity_session_adjustment(
            rows,
            row_index,
            cfg,
            decision_threshold,
        )
        pending_signal = _normalize_market_direction(score, adjusted_threshold, market_type)
        ...
        if pending_signal != 0 and (low_liquidity or low_dynamic_session):
            adjusted_multiplier = max(0.0, min(1.0, float(base_pending_meta.size_multiplier) * liquidity_size_multiplier))
            action = base_pending_meta.action
            if adjusted_multiplier <= 0.0:
                action = "skip"
            elif adjusted_multiplier < float(base_pending_meta.size_multiplier) and action == "take":
                action = "downsize"
            reasons = [base_pending_meta.reason]
            if low_liquidity:
            ...
            pending_meta = MetaLabelDecision(
                True,
                action,
                adjusted_multiplier,
                base_pending_meta.signal_strength,
                "; ".join(reason for reason in reasons if reason),
            )
        else:
            pending_meta = base_pending_meta
        price = row.close
