...
def render_equity_svg(points: Iterable[EquityPoint], *, title: str = "Backtest performance") -> str:
  ...
  <text x="{left}" y="{height - 20}" font-family="Segoe UI, Arial, sans-serif" font-size="13" fill="#334155">day-trading simulation timeline</text>
  ...
  <text x="{left}" y="{top + chart_h + 30}" font-family="Segoe UI, Arial, sans-serif" font-size="13" fill="#111827">start {equities[0]:.2f} | end {equities[-1]:.2f} | max drawdown {max_drawdown:.2%}</text>
</svg>
"""
