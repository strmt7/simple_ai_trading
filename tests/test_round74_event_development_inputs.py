...
class _Assembly:
    ...
    def from_dict(cls, value: object) -> _Assembly:
        if not isinstance(value, dict) or set(value) != {"run_id"}:
            raise ValueError("test assembly differs")
        run_id = str(value["run_id"])
        cls.loaded_run_ids.append(run_id)
        return cls(run_id)


def _write_panel(root: Path) -> tuple[Path, Path, Path]:
    plan = root / "plan.json"
    ...
    return plan, bindings, assemblies
...
def test_round74_development_input_loader_never_reads_sealed_assemblies(
    ...
    plan_path, bindings, assemblies = _write_panel(tmp_path)
    ...
    with pytest.raises(ValueError, match="assembly file panel differs"):
