...
CONTRACT = (
    ROOT
    / "docs/model-research/action-value/polymarket-or06-house-winner-complete-set-metadata-contract-v1-2026-09-01.json"
)
CAPTURE = (
    ROOT
    / "docs/model-research/action-value/polymarket-or06-house-winner-complete-set-metadata-capture-result-v1-2026-09-01.json"
)
RESULT = (
    ROOT
    / "docs/model-research/action-value/polymarket-or06-house-winner-complete-set-metadata-adjudication-v1-2026-09-01.json"
)
...
def test_or06_hidden_siblings_reject_apparent_two_party_complete_set() -> None:
    ...
    assert sum(bool(market["active"]) for market in markets) == 2
    assert sum(not bool(market["active"]) for market in markets) == 6
    assert {str(market["groupItemTitle"]) for market in markets} == {
        "Democratic Party",
        "Republican Party",
        "Other",
        "A",
    ...
    assert economics["complete_event_yes_ask_sum_pUSD_per_share"] == "6.971"
    ...
    family = "polymarket_OR_06_House_election_winner_hidden_inactive_NegRisk_partition_2026_09_01"
