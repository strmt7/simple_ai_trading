...
def test_round29_preregistration_is_hash_and_source_bound_before_targets() -> None:
    ...
    assert value["oracle_boundary"]["independent_twap_reconstruction_forbidden"]
...
def test_round29_preregistration_binds_exact_feature_and_comparison_contracts() -> None:
    ...
    assert feature["round27_base_feature_count"] == len(
        POLYMARKET_ROUND27_FEATURE_NAMES
    )
    assert feature["bbo_base_feature_count"] == len(POLYMARKET_ROUND28_FEATURE_NAMES)
    assert feature["settlement_feature_count"] == len(
        POLYMARKET_ROUND29_SETTLEMENT_FEATURE_NAMES
    )
    assert feature["settlement_augmented_feature_count"] == len(
        POLYMARKET_ROUND29_BASE_FEATURE_NAMES
    )
    assert feature["combined_feature_count"] == len(
        POLYMARKET_ROUND29_COMBINED_FEATURE_NAMES
    )
    assert (
        feature["settlement_feature_names_sha256"]
        == POLYMARKET_ROUND29_SETTLEMENT_FEATURE_NAMES_SHA256
    )
    assert (
        feature["settlement_augmented_feature_names_sha256"]
        == POLYMARKET_ROUND29_BASE_FEATURE_NAMES_SHA256
    )
    assert (
        feature["combined_feature_names_sha256"]
        == POLYMARKET_ROUND29_COMBINED_FEATURE_NAMES_SHA256
    )
    assert value["experiment"]["primary_pair"] == {
        "augmented": "round29_bbo_settlement_augmented_l2_offset_logistic",
        "base": "round28_bbo_augmented_l2_offset_logistic",
        "purpose": (
