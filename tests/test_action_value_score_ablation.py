...
from tools.ablate_action_value_scores import (
    ...
    _event_model_parameter_risk_level,
...
def test_score_policy_honors_explicit_viability_trade_floor() -> None:
    rows = 100
    timestamps = np.arange(rows, dtype=np.int64) * 10_000
    dataset = SimpleNamespace(
        decision_time_ms=timestamps,
        long_exit_time_ms=timestamps + 1_000,
        short_exit_time_ms=timestamps + 1_000,
        long_net_bps=np.where(np.arange(rows) >= 75, 10.0, -10.0),
        short_net_bps=np.full(rows, -10.0),
        long_liquidity_eligible=np.ones(rows, dtype=bool),
        short_liquidity_eligible=np.zeros(rows, dtype=bool),
    )
    ...
    assert policy["minimum_trades"] == 80
    assert policy["policies_meeting_trade_minimum"] == 0
    with pytest.raises(ValueError, match="minimum_trades must be positive"):
        _select_score_threshold(
            dataset=dataset,
            indexes=np.arange(rows, dtype=np.int64),
            long_scores=np.arange(rows, dtype=np.float64),
...
def test_bounded_viability_uses_one_shared_predictor_profile() -> None:
    assert _event_model_parameter_risk_level("regular", "risk_specific") == "regular"
    assert (
        _event_model_parameter_risk_level("aggressive", "shared_regularized")
        == "conservative"
    )
    with pytest.raises(ValueError, match="unsupported event-model parameter profile"):
        _event_model_parameter_risk_level("regular", "unknown")
