...
def test_registry_reopens_only_the_distinct_future_nba_recurrence_family() -> None:
    ...
    assert "monotone_payoff_implication" in row["mechanism"]
    ...
    assert all(
        artifact in row["canonical_artifacts"]
        for artifact in [
            {
                "path": (
                    "docs/model-research/action-value/"
    ...
    artifact_hashes = {item["result_sha256"] for item in row["canonical_artifacts"]}
    assert {
