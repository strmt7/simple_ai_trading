"""Screen one bounded future CFB catalog for exact monotone payoff packages."""
...
SCHEMA = "polymarket-cfb-monotone-catalog-result-v1"
...
def _validate_contract(contract: dict[str, Any], contract_path: Path) -> None:
    ...
    if capture != expected_capture:
        raise RuntimeError("CFB catalog population contract changed")
...
def main() -> None:
    ...
    raw, receipt = _request(
        ...
        body=b"",
        name="future-cfb-monotone-catalog",
        raw_path=raw_path,
    ...
    result: dict[str, Any] = {
        ...
        "implementation": {
            "path": "tools/screen_polymarket_cfb_monotone_catalog.py",
