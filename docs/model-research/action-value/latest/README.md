# Round 22: additive net-return ranking model abstained

**Rejected without trading authority.** The bounded additive pairwise term restored Round 20-like signal eligibility but did not improve the best out-of-sample tail, and every threshold-selection simulation remained negative net of stress costs. Signals meeting pre-threshold controls appeared only for Regular (17), Aggressive (147); Conservative produced none. The 8 resulting threshold candidates all failed the stress-test acceptance criteria, so no out-of-sample simulated trade, development access, leverage, or trading authority was permitted.
...
Probability-of-profit discrimination did not translate into an economically usable net-return ranking: threshold-selection stress ROC AUC reached 0.623, while every top-100 and top-500 realized mean net return remained negative. Further ranking-loss tuning is not justified; the next precommitted change must address target horizon or regime conditioning without relaxing the risk controls. The development window and reserved 2023-07-07 terminal day remain untouched.
