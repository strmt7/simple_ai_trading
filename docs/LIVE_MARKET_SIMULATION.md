...
- The autonomous open/stop helper also rejects `origQty`, requested local quantity, and fallback mark/entry prices as execution proof before writing a live ledger row.
- The signed `spot-roundtrip` test order command uses the same resolver for its first leg and will not place the second leg from an ACK-only response.
...
- `simple-ai-trading reconcile` compares signed account exposure with the local autonomous ledger. Paper positions are reported but ignored for exchange mismatch math; non-paper local positions must have bot-owned client-order and exchange fill/acknowledgement proof before they can match spot balances or futures positions for the active symbol set. Futures reconciliation requires a signed account payload with a `positions` list; spot reconciliation requires `balances`. The local `open_positions.json` file must also pass strict integrity checks for JSON shape, known fields, enums, finite quantities/prices/leverage/notional, and nonnegative timestamps before signed reconciliation math runs.
- Reconciliation writes `data/autonomous/reconciliation.json` and exits nonzero on exchange-only exposure, local-only exposure, unverified local live exposure, malformed account payloads, malformed local ledger files, or quantity mismatches.
...
- If reconciliation finds exchange-only exposure, local-only exposure,
  unverified local live exposure, malformed account payloads, malformed local
  ledger files, or a quantity mismatch, the loop exits fail-closed. It does not
  close exposure that is not represented in a verified bot-owned ledger row.
