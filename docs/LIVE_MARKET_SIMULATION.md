...
- `simple-ai-trading reconcile` compares signed account exposure with the local autonomous ledger. Paper positions are reported but ignored for exchange mismatch math; non-paper local positions must have bot-owned client-order and exchange fill/acknowledgement proof before they can match spot balances or futures positions for the active symbol set. Futures reconciliation requires a signed account payload with a `positions` list; spot reconciliation requires `balances`.
- Reconciliation writes `data/autonomous/reconciliation.json` and exits nonzero on exchange-only exposure, local-only exposure, unverified local live exposure, malformed account payloads, or quantity mismatches.
...
- If reconciliation finds exchange-only exposure, local-only exposure,
  unverified local live exposure, malformed account payloads, or a quantity
  mismatch, the loop exits fail-closed. It does not close exposure that is not
  represented in a verified bot-owned ledger row.
