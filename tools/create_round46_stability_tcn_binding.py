"""Bind the frozen Round 46 design, verified cache, and implementation."""
...
BOUND_PATHS = (
    "docs/model-research/action-value/round-046-stability-regularized-tcn-design.json",
    ...
    "tools/create_round46_stability_tcn_binding.py",
...
def _cache_manifest(
    ...
    if not isinstance(dataset, Mapping):
        raise ValueError("Round 45 predecessor report has no dataset section")
    reported = dataset.get("derived_cache")
    if not isinstance(reported, list):
        raise ValueError("Round 45 predecessor report has no cache manifest")
    by_name = {
    ...
    for name in CACHE_FILES:
        ...
        if item.get("sha256") != sha256 or item.get("bytes") != size:
            raise ValueError(f"Round 46 cache differs from Round 45: {name}")
        manifest.append(
...
def create(arguments: argparse.Namespace) -> dict[str, object]:
    if _git("status", "--porcelain"):
        raise ValueError("create the Round 46 binding only from a clean worktree")
    ...
    if source_sha != SOURCE_CANONICAL_SHA256:
        raise ValueError("Round 46 source certificate differs from the frozen source")
    if predecessor_sha != PREDECESSOR_REPORT_CANONICAL_SHA256:
        raise ValueError("Round 46 predecessor report differs from the frozen report")
    ...
    if not isinstance(data_contract, Mapping):
        raise ValueError("Round 46 design has no data contract")
    ...
    if (
        ...
        raise ValueError("Round 46 design and external lineage differ")
    ...
    binding: dict[str, object] = {
        ...
        "derived_cache": cache,
        "blobs": blobs,
        "command": ".venv311\\Scripts\\python.exe tools\\run_stability_regularized_tcn_viability.py --source-certificate <external-certificate.json> --predecessor-report <round45-report.json> --derived-cache <round45-derived-cache> --evidence-root <new-external-evidence-root> --compute-backend directml",
    }
