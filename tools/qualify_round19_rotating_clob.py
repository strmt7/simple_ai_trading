"""Run the preregistered Round 19 rotating public CLOB qualification."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a bounded dual-lane BTC Polymarket CLOB qualification with "
            "prospective market subscription rotation and no trading authority."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if requested_output.is_symlink() or output.exists():
        print(
            "round19-rotating-clob failed: output already exists or is a symlink",
            file=sys.stderr,
        )
        return 2
    try:
        result = run_round19_transport_qualification(
        ...
        print(f"round19-rotating-clob failed: {exc}", file=sys.stderr)
        return 2
    print(
        json.dumps(
            {
