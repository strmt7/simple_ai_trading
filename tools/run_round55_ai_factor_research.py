"""Generate Round 55 auditable factor programs with three local 8B models."""
...
REPORT_SCHEMA = "round-055-ai-factor-research-report-v1"
LEDGER_SCHEMA = "round-055-ai-factor-program-ledger-v1"
...
def _git(*arguments: str) -> str:
    try:
        ...
        raise ValueError("Round 55 AI Git identity command failed") from exc
...
def _validate_design(path: Path) -> tuple[dict[str, object], str]:
    ...
    if not isinstance(contract, Mapping):
        raise ValueError("Round 55 AI contract is absent")
    if (
        contract.get("models") != ["fin-r1:8b", "fino1:8b", "qwen3:8b"]
        or contract.get("factors_requested_per_model") != 4
        or contract.get("temperature") != 0.0
        or contract.get("seed") != 5500
        or contract.get("market_values_or_outcomes_visible_to_language_model")
        is not False
        or contract.get("allowed_functions") != list(ALLOWED_FUNCTIONS)
        or contract.get("order_authority") is not False
    ):
        raise ValueError("Round 55 AI implementation and frozen contract differ")
    return design, claimed


def _load_feature_metadata(path: Path) -> tuple[tuple[str, ...], dict[str, object]]:
    ...
    if (
        ...
        raise ValueError("Round 55 AI feature metadata is invalid")
    return tuple(names), {
...
def _prompt(feature_names: tuple[str, ...], factors: int) -> tuple[str, str]:
    system = (
        "You are a quantitative research assistant, not a trader. Produce only the "
        "requested strict JSON. You have no market values, labels, returns after the "
        "decision, backtest results, portfolio state, or order authority. Every proposed "
        "factor must be causal, interpretable, dimensionally defensible, and suitable as "
        "an input to a separate one-hour payoff model. Do not propose leverage, orders, "
        "position sizes, thresholds, or performance claims."
    )
    user = (
        f"Propose exactly {factors} distinct factor expressions for BTCUSDT, ETHUSDT, "
        "and SOLUSDT one-hour day-trading payoff research. Expressions may use only the "
        "feature names and operators below. Binary operators are +, -, and *. Raw /, **, "
        "comparisons, booleans, indexing, attributes, methods, and unknown names are "
        ...
        "^[a-z][a-z0-9_]{2,63}$. "
        "mechanism and failure_mode must each be a substantive sentence of at least 20 "
        "characters; placeholders such as 0, none, or unknown are forbidden. Use target "
        ...
        "Prefer economically different mechanisms across trend quality, flow confirmation, "
        "cross-asset residual behavior, volatility asymmetry, and liquidity-conditioned "
        "regimes. Avoid merely renaming an existing feature. expected_horizon must be "
        "one_hour. Output JSON only.\n\nAvailable features:\n- "
        + "\n- ".join(feature_names)
    )
    return system, user
...
def run(arguments: argparse.Namespace) -> int:
    ...
    implementation_blobs = {
        ...
        for path in (
            ...
            "tools/run_round55_ai_factor_research.py",
        )
    }
    feature_names, metadata_evidence = _load_feature_metadata(
    ...
    _progress(
        status_path,
        "contracts_validated",
        {"design_sha256": design_sha, "models": list(models)},
    )
    ...
    for model_index, model in enumerate(models, start=1):
        ...
        try:
            programs, program_rejections = parse_factor_response_ledger(
                text,
                model=model,
    ...
    ledger: dict[str, object] = {
        ...
        "rejections": rejected,
        "outcomes_or_market_values_read": False,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "docs/model-research/action-value/round-055-bounded-alpha-lightgbm-ai-design.json",
    )
