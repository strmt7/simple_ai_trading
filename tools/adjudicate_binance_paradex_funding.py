"""Adjudicate frozen Paradex USDC versus Binance USDT funding histories."""
...
SCHEMA = "binance-paradex-cross-venue-funding-prefilter-v1"
ZERO = Decimal(0)
...
def _validate_inventory(path: Path, *, assets: list[str], first_bucket_ms: int) -> None:
    payload = _load_object(path)
    rows = payload.get("results")
    if not isinstance(rows, list) or not rows:
        raise RuntimeError("Paradex market inventory envelope changed")
    by_symbol: dict[str, dict[str, Any]] = {}
    for row in rows:
        if not isinstance(row, dict):
            raise RuntimeError("Paradex market inventory row changed")
        symbol = row.get("symbol")
        if isinstance(symbol, str):
            if symbol in by_symbol:
                raise RuntimeError(f"duplicate Paradex market: {symbol}")
    ...
    for asset in assets:
        ...
        if (
            row is None
            or row.get("asset_kind") != "PERP"
            or row.get("base_currency") != asset
            or row.get("quote_currency") != "USD"
            or row.get("settlement_currency") != "USDC"
            or Decimal(str(row.get("funding_period_hours"))) != Decimal(8)
            or int(row.get("open_at", first_bucket_ms + 1)) > first_bucket_ms
            or int(row.get("expiry_at", -1)) != 0
            or row.get("trading_mode") != "STANDARD"
        ):
            raise RuntimeError(f"Paradex instrument identity failed for {symbol}")


def _paradex_series(path: Path, *, market: str) -> dict[int, Decimal]:
    payload = _load_object(path)
    rows = payload.get("results")
    if payload.get("next") not in (None, "") or not isinstance(rows, list) or not rows:
        raise RuntimeError(f"Paradex funding envelope changed for {market}")
    result: dict[int, Decimal] = {}
    for row in rows:
        if (
            ...
            raise RuntimeError(f"Paradex funding row changed for {market}")
        timestamp = int(row["created_at"])
        if timestamp % HOUR_MS or timestamp in result:
            raise RuntimeError(f"invalid Paradex hourly schedule for {market}")
        result[timestamp] = Decimal(str(row["funding_rate_8h"])) / Decimal(8)
    ordered = sorted(result)
    if any(right - left != HOUR_MS for left, right in zip(ordered, ordered[1:])):
        raise RuntimeError(f"incomplete Paradex hourly schedule for {market}")
    return result
...
def _role_result(spreads: list[Decimal], contract: dict[str, Any]) -> dict[str, Any]:
    ...
    sampling_bips = Decimal(economics["continuous_sampling_stress_bips"])
    net_bips = (
        gross_bips
        - execution_bips
        - capital_bips
    ...
    return {
        ...
        "continuous_sampling_stress_bips": str(sampling_bips),
...
def main() -> None:
    ...
    _validate_inventory(
        _root_path(contract["inventory"]["path"]),
        assets=population["assets"],
        first_bucket_ms=population["first_aligned_bucket_ms"],
    )
    aligned_by_asset: dict[str, list[tuple[int, Decimal, Decimal]]] = {}
    source_summaries: list[dict[str, Any]] = []
    for source in contract["sources"]:
        paradex = _paradex_series(
            _root_path(source["paradex"]["path"]),
            market=source["paradex"]["market"],
        )
        ...
        for bucket in range(
            ...
            if bucket not in binance or any(
                time not in paradex for time in hourly_times
            ):
                raise RuntimeError(f"frozen alignment incomplete for {source['asset']}")
            aligned.append(
                (
                    bucket,
                    sum((paradex[time] for time in hourly_times), ZERO),
                    binance[bucket],
                )
            )
        ...
        source_summaries.append(
            {
                "asset": source["asset"],
                "paradex_hourly_row_count": len(paradex),
    ...
    for asset in population["assets"]:
        ...
        training_delta = sum(
            (paradex - binance for _, paradex, binance in training_rows), ZERO
        )
        if training_delta >= ZERO:
            orientation = "short_paradex_USDC_long_binance_USDT"
            sign = Decimal(1)
        else:
            orientation = "long_paradex_USDC_short_binance_USDT"
            sign = Decimal(-1)
        role_results: dict[str, Any] = {}
        for role, bounds in contract["roles"].items():
            subset = rows[bounds["start"] : bounds["stop"]]
            spreads = [(paradex - binance) * sign for _, paradex, binance in subset]
    ...
    output: dict[str, Any] = {
        ...
        "adjudication": {
            "status": (
                "funding_prefilter_survivors_require_exact_index_confirmation"
                if survivors
