"""Bind the frozen Round 45 design, source certificate, and implementation."""
...
BOUND_PATHS = (
    "docs/model-research/action-value/round-045-joint-sam-tcn-design.json",
    ...
    "tools/create_round45_joint_sam_tcn_binding.py",
    "tools/run_joint_sam_tcn_viability.py",
...
def create(arguments: argparse.Namespace) -> dict[str, object]:
    if _git("status", "--porcelain"):
        raise ValueError("create the Round 45 binding only from a clean worktree")
    ...
    design_sha = _validate_canonical(
        design,
        field="design_sha256",
        expected_schema="joint-sam-distributional-tcn-design-v1",
    )
    ...
    if source_sha != SOURCE_CANONICAL_SHA256:
        raise ValueError("Round 45 source certificate differs from the frozen source")
    ...
    binding: dict[str, object] = {
        ...
        "blobs": blobs,
        "command": ".venv311\\Scripts\\python.exe tools\\run_joint_sam_tcn_viability.py --source-certificate <external-certificate.json> --evidence-root <new-external-evidence-root> --compute-backend directml",
    }
