...
DEFAULT_OUTPUT = Path(
    "docs/model-research/action-value/"
    "binance-two-intermediary-conversion-retained-v2-2026-08-29.json"
)
...
RESOURCE_ADJUDICATION_PATH = Path(
    "docs/model-research/action-value/"
...
def _load_contract() -> tuple[dict[str, object], str]:
    ...
    if contract.get("status") != "frozen_before_v2_retained_outcome_access":
        raise ValueError("v2 contract is not frozen")
    ...
    for path, field in checks:
        if _sha256(path.read_bytes()) != implementation[field]:
            raise ValueError(f"{path} differs from frozen v2 contract")
    prior = json.loads(RESOURCE_ADJUDICATION_PATH.read_text(encoding="ascii"))
    if (
        _canonical_hash(prior, "result_sha256")
        != contract["prior_failure"]["result_sha256"]
    ):
        raise ValueError("prior resource adjudication differs")
...
def run(output: Path) -> dict[str, object]:
    ...
    result_without_hash: dict[str, object] = {
        "schema_version": "binance-two-intermediary-conversion-retained-v2",
