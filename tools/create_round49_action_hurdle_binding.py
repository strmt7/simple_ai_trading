"""Bind the frozen Round 49 design, source certificate, and implementation."""
...
BOUND_PATHS = (
    ...
    "src/simple_ai_trading/action_hurdle_analysis.py",
    "src/simple_ai_trading/storage.py",
    "tools/run_action_hurdle_tcn_viability.py",
    "tools/create_round49_action_hurdle_binding.py",
    "tests/test_action_hurdle_tcn_model.py",
    "tests/test_action_hurdle_analysis.py",
)
...
def create(arguments: argparse.Namespace) -> dict[str, object]:
    if _git("status", "--porcelain"):
        raise ValueError("create the Round 49 binding only from a clean worktree")
    ...
    if source_sha != SOURCE_CANONICAL_SHA256:
        raise ValueError("Round 49 source certificate differs from the frozen source")
    ...
    if (
        ...
        raise ValueError("Round 49 design and source certificate differ")
    ...
    binding: dict[str, object] = {
        ...
        "blobs": blobs,
        "command": ".venv311\\Scripts\\python.exe tools\\run_action_hurdle_tcn_viability.py --database <market_data.sqlite> --source-certificate <external-certificate.json> --design docs\\model-research\\action-value\\round-049-cost-aware-action-hurdle-tcn-design.json --binding docs\\model-research\\action-value\\round-049-cost-aware-action-hurdle-tcn-binding.json --evidence-root <new-external-evidence-root> --compute-backend directml",
    }
