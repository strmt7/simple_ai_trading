"""Bind the frozen Round 50 design, source certificate, and implementation."""
...
BOUND_PATHS = (
    "docs/model-research/action-value/round-050-path-bounded-competing-risk-tcn-design.json",
    ...
    "src/simple_ai_trading/action_hurdle_tcn_model.py",
    "src/simple_ai_trading/barrier_payoff_data.py",
    "src/simple_ai_trading/barrier_competing_risk_tcn_model.py",
    "src/simple_ai_trading/barrier_competing_risk_analysis.py",
    "src/simple_ai_trading/storage.py",
    "tools/run_barrier_competing_risk_viability.py",
    "tools/create_round50_barrier_competing_risk_binding.py",
    "tests/test_barrier_payoff_data.py",
    "tests/test_barrier_competing_risk_tcn_model.py",
)
...
def create(arguments: argparse.Namespace) -> dict[str, object]:
    if _git("status", "--porcelain"):
        raise ValueError("create the Round 50 binding only from a clean worktree")
    ...
    if source_sha != SOURCE_CANONICAL_SHA256:
        raise ValueError("Round 50 source certificate differs from the frozen source")
    data_contract = design.get("data_contract")
    if (
        not isinstance(data_contract, Mapping)
        or data_contract.get("source_certificate_canonical_sha256") != source_sha
    ):
        raise ValueError("Round 50 design and source certificate differ")
    ...
    binding: dict[str, object] = {
        ...
        "blobs": blobs,
        "command": ".venv311\\Scripts\\python.exe tools\\run_barrier_competing_risk_viability.py --database <market_data.sqlite> --source-certificate <external-certificate.json> --design docs\\model-research\\action-value\\round-050-path-bounded-competing-risk-tcn-design.json --binding docs\\model-research\\action-value\\round-050-path-bounded-competing-risk-tcn-binding.json --evidence-root <new-external-evidence-root> --compute-backend directml",
    }
    binding["binding_sha256"] = _canonical_sha256(binding)
    write_json_atomic(arguments.output.resolve(), binding, indent=2, sort_keys=True)
    return binding
