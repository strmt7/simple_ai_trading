...
def _selected_rows(payload: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    selected = [
        ...
        and value.get("contractType") == "TRADFI_OPTIONS"
        and value.get("underlyingType") == "COMMODITY"
        and value.get("underlying") in {"XAGUSDT", "XAUUSDT"}
    ]
...
def main() -> int:
    ...
    selected = _selected_rows(_mapping(response.json(), name="exchangeInfo"))
    ...
    result: dict[str, object] = {
        "schema_version": "binance-commodity-option-expiry-delta-result-v1",
