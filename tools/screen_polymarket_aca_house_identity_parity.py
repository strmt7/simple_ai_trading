...
DATA_ROOT = ROOT / "data/polymarket-aca-house-identity-parity-v1"
...
def _validate_metadata(contract: dict[str, Any]) -> list[dict[str, Any]]:
    payloads: dict[str, dict[str, Any]] = {}
    for source in contract["metadata_sources"]:
        raw = (ROOT / source["path"]).read_bytes()
        if _sha256(raw) != source["sha256"]:
            raise RuntimeError(f"metadata hash mismatch: {source['name']}")
        payloads[source["name"]] = json.loads(raw)

    aca_market = payloads["aca"]["markets"][0]
    if not (
        payloads["aca"]["slug"] == contract["payoff_proof"]["aca_event_slug"]
        and payloads["aca"]["closed"] is True
        and aca_market["id"] == contract["payoff_proof"]["aca_market_id"]
        and aca_market["closed"] is True
        and aca_market["acceptingOrders"] is False
        and json.loads(aca_market["outcomePrices"]) == ["0", "1"]
        and aca_market["umaResolutionStatus"] == "resolved"
    ):
        raise RuntimeError("incorporated ACA market is not final No")

    combined_description = str(payloads["combined"].get("description") or "")
    ...
    for definition in contract["markets"]:
        event = payloads[definition["event"]]
        market = next(
...
def _fill(book: dict[str, Any], *, tick: Decimal) -> dict[str, Any] | None:
    levels = [
    ...
    remaining = QUANTITY
    actual_cost = stressed_cost = actual_fee = stressed_fee = Decimal("0")
    for price, available in levels:
        consumed = min(remaining, available)
        stressed_price = price + tick * 2
        if consumed <= 0:
            continue
        if stressed_price >= 1:
            return None
...
def main() -> None:
    ...
    for package in contract["packages"]:
        ...
        actual_net = stressed_net = None
        if left is not None and right is not None:
            actual_cost = Decimal(left["actual_cost_pUSD"]) + Decimal(
                right["actual_cost_pUSD"]
            )
            if actual_cost < QUANTITY and not set(package["tokens"]) <= fee_tokens:
                raise RuntimeError("gross-positive package lacks fee audit")
            actual_net = QUANTITY - actual_cost - Decimal(
                left["actual_fee_pUSD"]
            ) - Decimal(right["actual_fee_pUSD"])
            stressed_net = QUANTITY - Decimal(left["stressed_cost_pUSD"]) - Decimal(
                right["stressed_cost_pUSD"]
            ) - Decimal(left["stressed_fee_pUSD"]) - Decimal(
        ...
        rows.append(
            {
                **package,
                ...
                "passes_economic_gate": stressed_net is not None and stressed_net > 0,
                "passes_frozen_candidate_gate": synchronized
                and stressed_net is not None
                and stressed_net > 0,
            }
        )
    ...
    result: dict[str, Any] = {
        "schema_version": "polymarket-aca-house-identity-parity-result-v1",
        "contract": {
        ...
        "adjudication": {
            ...
            "next_action": "offline_maker_first_sensitivity_on_retained_batch_only",
        },
