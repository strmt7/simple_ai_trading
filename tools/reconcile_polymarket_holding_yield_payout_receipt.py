"""Reconcile one frozen Polymarket holding-yield row to its public receipt."""
...
def _exact_transfer(receipt: Mapping[str, object], contract: Mapping[str, object]) -> bool:
    expected = _mapping(contract["expected_transfer"], name="expected transfer")
    for raw_log in receipt.get("logs", []):
        ...
        if (
            str(log.get("address") or "").lower() == expected["token"]
            and len(topics) == 3
            and str(topics[0]).lower() == TRANSFER_TOPIC
            and _topic_address(topics[1]) == expected["from"]
            and _topic_address(topics[2]) == expected["to"]
            and Decimal(int(str(log.get("data")), 16)) / Decimal(1_000_000)
            == Decimal(str(expected["amount_pusd"]))
        ):
            return True
    return False
...
def run(
    *, contract_path: Path, raw_path: Path, journal_path: Path, output_path: Path
    ...
    if contract.get("schema_version") != (
        "polymarket-holding-yield-payout-receipt-v10-contract"
    ):
    ...
    journal: dict[str, object] = {
        "schema_version": "polymarket-holding-yield-payout-receipt-v10-journal",
        "state": "started",
    ...
    try:
        ...
        reconciled = (
            ...
            and _exact_transfer(receipt, contract)
        )
        if not reconciled:
            raise ValueError("frozen payout does not reconcile to an exact transfer")
        result: dict[str, object] = {
            "schema_version": "polymarket-holding-yield-payout-receipt-v10",
            "created_at_ms": time.time_ns() // 1_000_000,
            "purpose": "reconcile_one_new_BTC_holding_yield_row_to_exact_public_Polygon_pUSD_transfer",
            "contract": {
...
def main() -> int:
    ...
    raw_path = ROOT / (
        "data/polymarket-holding-yield-payout-receipt-v10/raw/btc-receipt.raw"
    )
    journal_path = ROOT / (
        "data/polymarket-holding-yield-payout-receipt-v10/journal.json"
    )
    output_path = ROOT / (
        "docs/model-research/polymarket/"
