"""Publish verified Round 41 prequential meta-label development evidence."""
...
from tools.run_prequential_meta_label_ai import (  # noqa: E402
    BINDING_SCHEMA,
    DESIGN_SCHEMA,
    REPORT_SCHEMA,
    _canonical_sha256,
...
PUBLICATION_SCHEMA = "prequential-meta-label-publication-v1"
ROUND = 41
...
def _validated_source(
    ...
    design = _read_object(design_path, "Round 41 design")
    design_sha = _canonical_identity(design, "design_sha256", "Round 41 design")
    binding = _read_object(binding_path, "Round 41 binding")
    binding_sha = _canonical_identity(binding, "binding_sha256", "Round 41 binding")
    report = _read_object(evidence_root / "report.json", "Round 41 report")
    report_sha = _canonical_identity(
        report, "report_canonical_sha256", "Round 41 report"
    )
    cases = _read_object(evidence_root / "ai-cases.json", "Round 41 AI case set")
    if (
        ...
        raise ValueError("Round 41 evidence lineage is invalid")
    ...
    if (
        ...
        or dataset.get("primary_feature_count") != 71
        or dataset.get("meta_feature_count") != 81
        or dataset.get("features_dtype") != "float32"
        or dataset.get("persistent_feature_or_prediction_copy_created") is not False
        or not isinstance(candidate, Mapping)
        or candidate.get("selected_threshold_months") != 2
        or candidate.get("months_with_trades") != 2
        ...
        or len(report.get("primary_schedules", [])) != 14
        or len(report.get("meta_schedules", [])) != 6
        ...
        or cases.get("schema_version") != "prequential-meta-label-ai-veto-case-set-v1"
        or cases.get("cases") != []
    ):
        raise ValueError("Round 41 source or model evidence drifted")
    aggregate = candidate["aggregate_replay"]
    if (
        aggregate.get("total_trades") != 466
        or abs(float(aggregate["mean_net_bps"]) - (-0.6773770308609685)) > 1e-12
        or abs(float(aggregate["profit_factor"]) - 0.9803029043725825) > 1e-12
        or abs(
        ...
        raise ValueError("Round 41 aggregate economics drifted")
    for field in (
        "trading_authority",
        "execution_claim",
        "profitability_claim",
        ...
        if report.get(field) is not False:
            raise ValueError(f"Round 41 report unexpectedly claims {field}")
        ...
        rows.append(
            {
                ...
                "economic_threshold_cells": sum(
                    item["economic_gate_passed"] for item in month["threshold_trace"]
                ),
                ...
                "meta_early_stop_roc_auc": month["meta_classification"]["early_stop"][
                    "roc_auc"
                ],
                "meta_calibration_roc_auc": month["meta_classification"][
                    "threshold_calibration"
                ]["roc_auc"],
                "meta_evaluation_roc_auc": month["meta_classification"]["evaluation"][
                    "roc_auc"
                ],
                ...
                "overlap_rejections": month["evaluation_replay"]["overlap_rejections"],
            }
        )
    return rows
    ...
    for month in report["candidate_result"]["monthly_results"]:
        ...
        for item in month["threshold_trace"]:
            ...
            rows.append(
                {
                    "evaluation_month": month["evaluation_month"],
                    "meta_probability_threshold": item["meta_probability_threshold"],
                    ...
                    "overlap_rejections": item["capacity_replay"]["overlap_rejections"],
...
def _progress_rows(
    ...
    if not fields or observed not in (list(range(1, 41)), list(range(1, 42))):
        raise ValueError("Round 41 prior progress history is invalid")
    ...
    row.update(
        {
            "round": ROUND,
            "stage": "six-month prequential profitability meta-label",
            "periods": "2023-11-01..2024-12-31 rolling OOF roles",
            "selection_contaminated": True,
            "horizon_seconds": "1800",
            "feature_set": "71 primary + 10 prequential meta features",
            ...
            "status": "rejected",
            "source_file": "verified Round 41 prequential meta-label report",
...
def _auc_svg(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    lines = _svg_start(
        width,
        height,
        "Longer prequential history reduced fit inflation, not regime instability",
        "Binary profitability ROC AUC by causal role; fit uses six prior out-of-fold months while every later role remains unseen.",
    )
...
def _calibration_svg(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    lines = _svg_start(
        width,
        height,
        "Late calibration cleared uncertainty, then failed out of sample",
        "Best support-complete threshold per month; November and December labels refer to gates passed before the following evaluation month.",
    )
    ...
    for index, row in enumerate(rows):
        ...
        lines.append(f'<circle cx="{x:.1f}" cy="{y(mean):.1f}" r="8" fill="{color}"/>')
        lines.append(
...
def _evaluation_svg(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    lines = _svg_start(
        width,
        height,
        "Both selected evaluation months lost after the frozen cost charge",
    ...
    for index, row in enumerate(rows):
        ...
        if int(row["evaluation_trades"]) == 0:
            ...
            value_y = y(value)
            color = "#0f766e" if value > 0.0 else "#b42318"
            lines.append(
                f'<rect x="{x - 42:.1f}" y="{min(zero, value_y):.1f}" width="84" height="{abs(zero - value_y):.1f}" fill="{color}"/>'
            )
            label_y = value_y - 15.0 if value >= 0.0 else value_y + 28.0
            lines.append(
                f'<text x="{x:.1f}" y="{label_y:.1f}" text-anchor="middle" class="value">{value:+.2f} bps; n={int(row["evaluation_trades"])}</text>'
            )
        lines.append(
...
def _readme(
    report: Mapping[str, object], monthly: Sequence[Mapping[str, object]]
) -> str:
    aggregate = report["candidate_result"]["aggregate_replay"]
    auc = [float(row["meta_evaluation_roc_auc"]) for row in monthly]
    runtime = report["runtime_evidence"]
    return f"""# Round 41: prequential meta-label screen rejected
...
def publish(
    *,
    ...
    publication: dict[str, object] = {
        ...
        "artifact_class": "prequential_meta_label_graph_data",
        "round": ROUND,
        "status": "rejected",
        ...
        "primary_feature_count": report["dataset"]["primary_feature_count"],
        ...
        "aggregate_total_net_bps": report["candidate_result"]["aggregate_replay"][
            "total_net_bps"
        ],
        "aggregate_profit_factor": report["candidate_result"]["aggregate_replay"][
            "profit_factor"
        ],
        "aggregate_day_block_lower_95_mean_net_bps": report["candidate_result"][
            "aggregate_replay"
        ]["day_block_bootstrap_mean_net_bps_lower_95"],
        "ai_case_count": report["ai_case_set"]["cases"],
        "selection_contaminated": True,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--design",
        type=Path,
        default=research / "round-041-prequential-meta-label-ai-design.json",
    )
    parser.add_argument(
        "--binding",
        type=Path,
        default=research / "round-041-prequential-meta-label-execution-binding.json",
    )
    parser.add_argument(
