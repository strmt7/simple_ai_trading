"""Adjudicate frozen Lighter USDC versus Binance USDT funding histories."""
...
SCHEMA = "binance-lighter-cross-venue-funding-prefilter-v1"
ZERO = Decimal(0)
...
def _lighter_series(path: Path, *, expected_rows: int) -> dict[int, Decimal]:
    payload = _load_object(path)
    rows = payload.get("fundings")
    if (
        payload.get("code") != 200
        or payload.get("resolution") != "1h"
        or not isinstance(rows, list)
        or len(rows) != expected_rows
    ):
        raise RuntimeError(f"Lighter funding envelope changed: {path.name}")
    result: dict[int, Decimal] = {}
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
            ...
            raise RuntimeError(f"Lighter funding row shape changed: {path.name}")
        timestamp = int(row["timestamp"]) * 1000
        if timestamp % HOUR_MS or timestamp in result:
            raise RuntimeError(f"invalid Lighter hourly schedule: {path.name}")
        magnitude_percent = Decimal(str(row["rate"]))
        direction = row["direction"]
        if magnitude_percent < ZERO or direction not in {"long", "short"}:
            raise RuntimeError(f"invalid Lighter rate semantics: {path.name}")
        # The endpoint separates magnitude (percentage points) from payer side.
        # Positive normalized rates mean longs pay shorts, matching Binance.
        sign = Decimal(1) if direction == "long" else Decimal(-1)
        result[timestamp] = sign * magnitude_percent / Decimal(100)
    ordered = sorted(result)
    if any(right - left != HOUR_MS for left, right in zip(ordered, ordered[1:])):
        raise RuntimeError(f"incomplete Lighter hourly schedule: {path.name}")
    return result
...
def _role_result(spreads: list[Decimal], contract: dict[str, Any]) -> dict[str, Any]:
    ...
    custody_bips = Decimal(economics["custody_bridge_latency_stress_bips"])
    ...
    return {
        ...
        "custody_bridge_latency_stress_bips": str(custody_bips),
...
def main() -> None:
    ...
    for source in contract["sources"]:
        lighter = _lighter_series(
            _root_path(source["lighter"]["path"]),
            expected_rows=population["lighter_hourly_row_count"],
        )
        ...
        for bucket in range(
            ...
            if bucket not in binance or any(time not in lighter for time in hourly_times):
                raise RuntimeError(f"frozen alignment incomplete for {source['asset']}")
            aligned.append(
                (bucket, sum((lighter[time] for time in hourly_times), ZERO), binance[bucket])
            )
        ...
        source_summaries.append(
            {
                "asset": source["asset"],
                "lighter_hourly_row_count": len(lighter),
    ...
    for asset in population["assets"]:
        ...
        training_delta = sum(
            (lighter - binance for _, lighter, binance in training_rows), ZERO
        )
        if training_delta >= ZERO:
            orientation = "short_lighter_USDC_long_binance_USDT"
            sign = Decimal(1)
        else:
            orientation = "long_lighter_USDC_short_binance_USDT"
            sign = Decimal(-1)
        role_results: dict[str, Any] = {}
        for role, bounds in contract["roles"].items():
            subset = rows[bounds["start"] : bounds["stop"]]
            spreads = [(lighter - binance) * sign for _, lighter, binance in subset]
    ...
    output: dict[str, Any] = {
        ...
        "adjudication": {
            "status": (
                "funding_prefilter_survivors_require_disjoint_confirmation"
                if survivors and not basis_justified
                else "funding_prefilter_survivors_require_basis_and_depth"
                if basis_justified
