"""Run the hash-bound Round 41 prequential meta-label and finance-AI screen."""
...
from simple_ai_trading.prequential_meta_label_model import (  # noqa: E402
    EVALUATION_MONTHS,
...
ROUND = 41
DESIGN_SCHEMA = "prequential-meta-label-ai-design-v1"
BINDING_SCHEMA = "round-041-prequential-meta-label-execution-binding-v1"
REPORT_SCHEMA = "prequential-meta-label-ai-report-v1"


def _validate_design(path: Path) -> tuple[dict[str, object], str]:
    design = _read_object(path, "Round 41 design")
    ...
    primary = design.get("prequential_primary_panel")
    meta_walk = design.get("meta_walk_forward")
    ...
    if not all(
        ...
        raise ValueError("Round 41 design sections are incomplete")
    for field in (
        ...
        if governance.get(field) is not False:
            raise ValueError(f"Round 41 governance must deny {field}")
    if (
        source.get("symbols") != list(SYMBOLS)
        or source.get("dataset_rows") != 1_098_105
        or source.get("meta_feature_count") != META_FEATURE_COUNT
        or primary.get("target_months") != list(PRIMARY_TARGET_MONTHS)
        or primary.get("primary_models") != 42
        or meta_walk.get("evaluation_months") != list(EVALUATION_MONTHS)
        ...
        raise ValueError("Round 41 implementation and design contracts differ")
...
def _validate_binding(
    ...
    *,
    ...
    binding = _read_object(path, "Round 41 execution binding")
    ...
    if (
        ...
        raise ValueError("Round 41 execution binding identity is invalid")
    ...
    if (
        ...
        raise ValueError("Round 41 source certificate binding is invalid")
    implementation_commit = str(binding.get("implementation_commit") or "")
    if len(implementation_commit) != 40:
        raise ValueError("Round 41 implementation commit is invalid")
    if _git("status", "--porcelain"):
        raise ValueError("Round 41 execution requires a clean worktree")
    try:
        subprocess.run(
            [
                "git",
        ...
        raise ValueError("Round 41 implementation is not an ancestor of HEAD") from exc
    blobs = binding.get("blobs")
    if not isinstance(blobs, list) or not blobs:
        raise ValueError("Round 41 bound blobs are missing")
    for item in blobs:
        if not isinstance(item, Mapping):
            raise ValueError("Round 41 bound blob is invalid")
        ...
        if (
            ...
            raise ValueError(f"Round 41 bound blob changed: {relative_path}")
...
class ProgressWriter:
    ...
    def __call__(self, phase: str, detail: Mapping[str, object]) -> None:
        self.sequence += 1
        payload = {
            "schema_version": "round-041-progress-v1",
            "round": ROUND,
...
def run(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    screen = run_prequential_meta_label_screen(
        dataset,
    ...
    case_payload = {
        "schema_version": "prequential-meta-label-ai-veto-case-set-v1",
    ...
    report: dict[str, object] = {
        ...
        "dataset": {
            "rows": dataset.rows,
            "feature_count": len(dataset.feature_names),
            "primary_feature_count": dataset.price_flow_feature_count,
            "meta_feature_count": META_FEATURE_COUNT,
            "features_dtype": str(dataset.features.dtype),
            "features_bytes": int(dataset.features.nbytes),
            "source_exclusions": dict(dataset.source_exclusions),
            "horizon_minutes": 30,
            "persistent_feature_or_prediction_copy_created": False,
        },
        "primary_schedules": list(screen.primary_schedules),
        "meta_schedules": list(screen.meta_schedules),
