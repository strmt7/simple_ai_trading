"""Bind the frozen Round 44 design, source certificate, and implementation."""
...
BOUND_PATHS = (
    "docs/model-research/action-value/round-044-distributional-tcn-design.json",
    ...
    "tools/create_round44_distributional_tcn_binding.py",
    "tools/run_distributional_tcn_viability.py",
...
def create(arguments: argparse.Namespace) -> dict[str, object]:
    if _git("status", "--porcelain"):
        raise ValueError("create the Round 44 binding only from a clean worktree")
    ...
    design_sha = _validate_canonical(
        design,
        field="design_sha256",
        expected_schema="distributional-tcn-viability-design-v1",
    )
    ...
    if source_sha != SOURCE_CANONICAL_SHA256:
        raise ValueError("Round 44 source certificate differs from the frozen source")
    ...
    binding: dict[str, object] = {
        ...
        "blobs": blobs,
        "command": ".venv311\\Scripts\\python.exe tools\\run_distributional_tcn_viability.py --source-certificate <external-certificate.json> --evidence-root <new-external-evidence-root> --compute-backend directml",
    }
