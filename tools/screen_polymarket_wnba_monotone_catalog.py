"""Screen one bounded future WNBA catalog for exact monotone payoff packages."""
...
SCHEMA = "polymarket-wnba-monotone-catalog-result-v1"
...
def _validate_contract(contract: dict[str, Any], contract_path: Path) -> None:
    ...
    if _instant(capture["start_time_min"], "start_time_min") <= frozen:
        raise RuntimeError("catalog must begin after the frozen instant")
    if _instant(capture["start_time_max"], "start_time_max") <= _instant(
        capture["start_time_min"], "start_time_min"
    ):
    ...
    if capture != {
        ...
        "request_count": 1,
        "series_id": "10105",
        "start_time_max": capture["start_time_max"],
        "start_time_min": capture["start_time_min"],
        "tag_slug": "wnba",
    }:
        raise RuntimeError("catalog population contract changed")
...
def _catalog_url(capture: dict[str, Any]) -> str:
    query = urlencode(
        {
            "closed": str(capture["closed"]).lower(),
            "tag_slug": capture["tag_slug"],
            "series_id": capture["series_id"],
            "start_time_min": capture["start_time_min"],
            "start_time_max": capture["start_time_max"],
            "limit": capture["limit"],
            "order": capture["order"],
            "ascending": str(capture["ascending"]).lower(),
        }
    )
    return f"https://gamma-api.polymarket.com/events/keyset?{query}"
...
def _margin_markets(
    ...
    required = (
        ...
        "final score including any overtime periods.",
    )
    if not all(fragment in description for fragment in required):
        raise RuntimeError("WNBA moneyline lacks exact win overtime or cancellation rules")
    rows = [
        _market_ref(
            moneyline,
            threshold=1,
            ...
            resolver="integer_margin_moneyline_no_final_tie",
        )
    ]
    for market in markets:
        ...
        threshold = required_margin if favored == team_a else -(required_margin - 1)
        prices = dict(
            zip(
...
def main() -> None:
    ...
    raw, receipt = _request(
        method="GET",
        url=_catalog_url(contract["capture"]),
        body=b"",
        name="future-wnba-monotone-catalog",
        raw_path=raw_path,
    ...
    result: dict[str, Any] = {
        ...
        "implementation": {
            "path": "tools/screen_polymarket_wnba_monotone_catalog.py",
