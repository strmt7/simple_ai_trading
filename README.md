...
- The current Windows beta can use `torch-directml` for PyTorch training on compatible DirectX 12 GPUs, including the tested AMD host. DirectML is a compatibility backend, not a deployment or vendor assumption. Accepted portable models require a deterministic CPU reference and explicit ONNX provider discovery; a future Windows inference path should prefer Windows ML when it is actually implemented and verified.
...
With the optional Windows DirectML training extra installed, a compatible GPU may resolve to `compute=directml`. Provider discovery must report what actually ran; the application may not infer AMD, NVIDIA, Intel, CUDA, ROCm, DirectML, or CPU capability from the operating system alone. This host was verified with `torch-directml` on an AMD Radeon GPU using a real tensor operation on `privateuseone:0`.
