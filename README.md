...
`data-health` audits the SQLite market database before training or optimization. It reports row counts, UTC spans, expected rows, gap count, coverage ratio, archive-file status counts, and checksum-status counts for every stored symbol/market/interval or for an explicit symbol batch. It exits nonzero when minimum rows, coverage, gap, archive-error, checksum-mismatch, or `--require-verified-checksum` gates fail.
