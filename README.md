...
`ai-review` sends a compact, redacted model-lab report to a local structured-output Ollama model and writes `ai_risk_review.json`. It is an advisory risk review with fail-closed output: deterministic model-lab/portfolio failures, positive hybrid or feature ablation deltas that show a selected component is hurting the accepted score, missing GPU AI capability, unavailable providers, or invalid model JSON all produce a veto/review-required result rather than an approval.
