...
- The app has direct buttons for Stop + Close, Pause, Reconcile, Positions, and Risk Review. Stop + Close independently latches both Binance and Polymarket runtimes and still attempts the second shutdown if the first fails. Normal workflow cards are distinct paper, research, graph, data, and settings tasks so the UI does not present multiple similar buttons for the same action.
