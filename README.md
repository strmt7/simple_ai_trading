...
`0.1.0-beta.1` is a beta research release. Many advanced AI, live-trading, high-frequency archive, optimization, and Windows-app workflows are still incomplete or non-functional in production terms. Binance execution remains testnet/demo-first. Polymarket has an independent BTC-only live-capable execution boundary, but it is disabled by default and has no promoted model, authenticated host proof, or release authority. Do not rely on this software to protect capital or produce profit.
...
- Polymarket research and execution are BTC-only and independent from Binance accounts and execution. The latest one-hour Round 14 shadow rejected the current first-candidate policy: 3/12 wins, `-9.87720` counterfactual after-cost quote P&L, `0.4557` profit factor, and `11.69847` quote maximum drawdown. It placed no orders. Round 15 is the preregistered 154-day five-minute study; Round 16 preregisters a separate fifteen-minute comparison after current research reported settlement manipulation in five-minute BTC contracts. Binance BTCUSDT spot and perpetual data are optional public predictor inputs only. The [Polymarket live boundary](docs/POLYMARKET_LIVE_EXECUTION.md) owns its wallet, orders, fills, positions, risk ledger, reconciliation, stop, and settlement state, including dedicated-wallet order identity, per-market maker/taker fee evidence, and finalized on-chain redemption-payout proof. Its autonomous operator path is wired but fails closed because no model has passed promotion or received live authority.
- Round 21 is a target-blind 30-day BTC five-minute capture, not an open-ended optimizer. Its frozen program compares five deterministic candidates and 81 after-cost replay ledgers. The local-AI gate compares exactly three immutable 8B candidates on the same development population, nominates at most one challenger for a one-use sealed test, and returns no challenger if every profile/scenario gate does not pass. It grants no paper or live authority.
- Binance impact-absorption Round 73 is invalidated before target or model access because its v9 resource gate selected on market message activity. Its 18 indexed campaign hours cannot be used for modeling. Round 74/v10 is the only open exact-wire capture path; it separates data quality from host safety and never uses bytes per message to admit a market interval. No Round 74 model or performance result exists.
...
The Windows app is a native C++20 Win32 operator workstation inspired by the SuperZip structure: DPI-aware layout, native controls, dark window chrome, and Overview, Trading, Research, Risk, Data, System, and Settings pages. Overview keeps Start, Pause, separate Binance and Polymarket Stop controls, risk profile, execution mode, leverage, AI, and reinvestment controls together. CLI commands and options are generated from `argparse`; one shared taxonomy places every command in a deliberate workflow group. Generation and parity tests fail on missing, duplicate, or stale entries, while Settings retains the complete expert catalog.
...
- The app has separate `Stop Binance` and `Stop Polymarket` buttons plus Pause, Reconcile, Positions, and Risk Review. Each Stop has its own worker gate and invokes only its venue-specific backend, so a slow or failed Binance control cannot block Polymarket Stop, and vice versa. Normal workflow cards are distinct paper, research, graph, data, and settings tasks so the UI does not present multiple similar buttons for the same action.
